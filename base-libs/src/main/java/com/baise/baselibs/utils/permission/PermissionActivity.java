...
public class PermissionActivity extends AppCompatActivity {
    ...
    private final String defaultEnsure = "打开权限";
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PERMISSION_REQUEST_CODE && PermissionsUtil.isGranted(grantResults) && PermissionsUtil.hasPermission(this, permissions)) {
            permissionsGranted();
        } else if (showTip) {
    ...
    private void showMissingPermissionDialog() {
        ...
        new CircleDialog.Builder().setWidth(0.8f).setTitle(TextUtils.isEmpty(tipInfo.title) ? defaultTitle : tipInfo.title).
                setText(TextUtils.isEmpty(tipInfo.content) ? defaultContent : tipInfo.content).
                setPositive(TextUtils.isEmpty(tipInfo.ensure) ? defaultEnsure : tipInfo.ensure, v ->
                        //确认按钮
                        PermissionsUtil.gotoSetting(PermissionActivity.this)).
                setNegative(TextUtils.isEmpty(tipInfo.cancel) ? defaultCancel : tipInfo.cancel, v ->
                        //取消按钮
                        permissionsDenied()).
